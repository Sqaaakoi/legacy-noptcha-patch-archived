...
class Time {
    ...
    static string(t = null) {
        return t = t || Time.date(),
            Time.pad(t.getMonth() + 1) + `/${Time.pad(t.getDate())}/${t.getFullYear()} ${Time.pad(t.getHours() % 12)}:${Time.pad(t.getMinutes())}:${Time.pad(t.getSeconds())} ` + (12 <= t.getHours() ? "PM" : "AM")
    }
}
class BG {
    static exec(t, r) {
    ...
    static encode(e) {
        return new Promise(r => {
            ...
            t.onload = () => {
                ...
                e.onloadend = () => {
                    let t = e.result;
                    if (t.startsWith("data:text/html;base64,"))
                        return r(null);
