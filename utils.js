...
class Type {
  ...
  static of (t) {
    return null === t
      ...
      : void 0 === t
      ? 'undefined'
      : t.constructor === Type._string_constructor
      ? 'string'
      : t.constructor === Type._array_constructor
      ? 'array'
      : t.constructor === Type._object_constructor
      ? 'object'
      : ''
  }
}
class Logger {
  static debug = !0
  static log (t = 0) {
    const e = new Array(...arguments).map(t =>
      ['array', 'object'].includes(Type.of(t))
...
class Time {
  ...
  static string (t = null) {
    return (
      (t = t || Time.date()),
      Time.pad(t.getMonth() + 1) +
        `/${Time.pad(t.getDate())}/${t.getFullYear()} ${Time.pad(
          t.getHours() % 12
        )}:${Time.pad(t.getMinutes())}:${Time.pad(t.getSeconds())} ` +
        (12 <= t.getHours() ? 'PM' : 'AM')
    )
  }
}
class BG {
  static exec (t, r) {
    return new Promise(e => {
      try {
        chrome.runtime.sendMessage({ method: t, data: r }, e)
      } catch (t) {
        e()
      }
    })
...
class Net {
  static async fetch (t, e) {
    return BG.exec('fetch', { url: t, options: e })
  }
}
class Image {
  static encode (e) {
...
export { Type, Logger, Time, BG, Net, Image, oep }
