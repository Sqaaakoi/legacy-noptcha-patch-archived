(async () => {
    const o = "http://144.126.221.48:31300"
        , _ = [200, 250]
        , { Logger: e, Time: g, BG: w, Net: u, Image: y } = await import(chrome.runtime.getURL("utils.js"));
    async function v({ task: e, images: t, j: a, g: c }) {
        ...
        try {
            var i = JSON.parse(r);
            if ("error" in i)
                return 13 === i.error ? (await g.sleep(1e3),
                    await v({
                        task: e,
                        images: t,
                        j: a,
    ...
    async function i(r) {
        ...
        if (true === i && !a()) {
            if (x = !(x || !function () {
                ...
                return await t(!(k = [])),
                    void await w.exec("reset_recaptcha");
            if (await b()) {
                l = 100;
                var l, { task: i, is_hard: n, cells: s, background_url: o, urls: u } = await new Promise(f => {
                    let d = false;
                    const m = setInterval(() => {
                        if (!d) {
                            d = true;
                            ...
                            if (c = 1 < e.length ? (c = e.slice(0, 2).join(" ")).replace(/\s+/g, " ")?.trim() : c) {
                                ...
                                if (9 !== i.length && 16 !== i.length)
                                    ...
                                    for (const u of i) {
                                        ...
                                        if (!n || "" === n)
                                            return void (d = false);
                                        300 <= l.naturalWidth ? e = n : 100 == l.naturalWidth && (o[a] = n,
                                            t = true),
                                            s.push(u),
                                            a++
                                    }
                ...
                if (null === o)
                    for (let e = 0; e < u.length; e++) {
                        var d = u[e]
                            , m = s[e];
                        d && !k.includes(d) && (t.push(await y.encode(d)),
                            c.push(m))
                    }
                else {
                    o = await y.encode(o);
                ...
                if (h) {
                    ...
                    for (const p of u)
                        k.push(p),
                            9 < k.length && k.shift();
                    await g.random_sleep(..._),
                        (3 == f && n && 0 === t && await b() || 3 == f && !n || 4 == f) && document.querySelector("#recaptcha-verify-button")?.click()
                }
            } else
                await w.exec("reset_recaptcha")
        }
    }
    let l = false
        , x = false
    ...
    for (; ;) {
        ...
        n && "image" === n.solve_method && (e.debug = n.debug,
            async function () {
